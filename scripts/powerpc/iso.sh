...
FREEBSD_LABEL=${FREEBSD_LABEL:-"FreeBSD"}
...
echo "/dev/iso9660/${FREEBSD_LABEL} / cd9660 rw 0 0" > ${BASEDIR}/etc/fstab
...
mkisofs -hfs-bless boot -map ${SRCDIR}/release/powerpc/hfs.map -r -hfs -part -no-desktop -hfs-volid ${FREEBSD_LABEL} -V ${FREEBSD_LABEL} -l -J -L -o $ISOPATH . >> ${LOGFILE} 2>&1
