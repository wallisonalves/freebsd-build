...
# If the FREEBSD_DEBUG environment variable is set, be verbose.
[ ! -z "${FREEBSD_DEBUG:-}" ] && set -x
...
if [ -n "${1:-}" ]; then
    ...
    LOGFILE=$(mktemp -q /tmp/freebsd.XXXXXX)
    REMOVELOG=1
fi

cd $CURDIR

. ./conf/freebsd.defaults.conf

FREEBSD_CONF=${FREEBSD_CONF:-./conf/freebsd.conf}

[ -f ${FREEBSD_CONF} ] && . ${FREEBSD_CONF}
...
report_error() {
    if [ ! -z ${FREEBSD_ERROR_MAIL:-} ]; then
	cat ${LOGFILE} | \
	    mail -s "FreeBSD build error in ${TARGET} phase" \
	    ${FREEBSD_ERROR_MAIL}
    fi
}
