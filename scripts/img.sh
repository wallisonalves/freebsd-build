...
make_bios_img()
{
echo "/dev/ufs/${FREEBSD_LABEL} / ufs ro,noatime 1 1" > ${BASEDIR}/etc/fstab
...
makefs -B little -o label=${FREEBSD_LABEL} ${IMGPATH} ${BASEDIR}
if [ $? -ne 0 ]; then
  echo "makefs failed"
  exit 1
fi
...
makefs -B little -o label=$FREEBSD_LABEL ${UEFI_IMGPATH}.part ${BASEDIR}
if [ $? -ne 0 ]; then
